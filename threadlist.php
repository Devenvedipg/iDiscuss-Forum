...
<body>
    ...
    <?php 
        ...
        //   echo $method;
          ...
          $sql = "INSERT INTO `threads` ( `thread_title`, `thread_desc`, `thread_cat_id`, `thread_user_id`) VALUES ( '$th_title', '$th_desc', '$cat_id', '$sno')";
          $result = mysqli_query($conn,$sql);
           $showAlert = true;
             if($showAlert){
    ...
    <!-- Category container starts here with quidelines -->
    ...
    <!-- Add a thread title and thread description inside form -->
    <?php
    ...
    <!-- Query for fetching threads title and thread description from threads decription -->
        ...
        <?php 
          ...
          while( $row = mysqli_fetch_assoc($result) ){
              ...
              <div class="media-body">'.
               '<h5 class="mt-0"> <a class="text-dark" href="thread.php?threadid=' . $tid. '">'. $ttitle . ' </a></h5>
                  '. $tdesc . ' </div>'.'<div class="font-weight-bold my-0"> <b>Asked by:</b> '. $row2['user_email'] . ' at '. $thread_time. '</div>'.
          '</div>';
  
    }
       // If no threads found inside the category ,then if condition will get executed
         if($noResult){
            echo'<div class="jumbotron jumbotron-fluid">
            <div class="container">
              <p class="display-7">No Threads Found</p>
